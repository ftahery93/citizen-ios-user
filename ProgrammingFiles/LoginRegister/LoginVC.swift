...
class LoginVC: Base {
    ...
    override func viewDidLoad() {
        ...
        mobileNumberTF.heightAnchor.constraint(equalToConstant: 114*widthFactor).isActive = true
        mobileNumberTF.placeholder = "Email or Mobile".localized()
        ...
        passwordTF.heightAnchor.constraint(equalToConstant: 114*widthFactor).isActive = true
    ...
    @objc func loginButtAction(){
        print("login pressed")
        if( mobileNumberTF.text?.count == 0){
            showToast(toastView: self.view, toastTitle: "myErr".localized(), toastText: "err002".localized())
            return
        }
    ...
    func loginNow() {
        ...
        if (isConnectedToInternet()) {
            ...
            var params : [String: Any] = [
                "username_or_email": mobileNumberTF.text!,
                "password": passwordTF.text!,
                "fields" : "id,email",
                "current_customer_id" : myGlobalUserId
            ]
            
            
            showLoader()
            
            API.callDictionaryAPI(webserviceFor: StringIdentifiers.make_login, webMethod: .get, parameters: params, completion: { (error:Error?, task:Any?) in
                ...
                if let tasks = task {
                    ...
                    if (authorizationResponse.customers![0].id == -1) {
                        
                        
                    } else {
                        
                        if (authorizationResponse.customers![0].vendor_id ?? 0 >= 1) {
                        ...
                        isloggedin = true
                        print("userid2====\(authorizationResponse.customers![0].id!)")
                        UserDefaults.standard.setValue("\(authorizationResponse.customers![0].id!)", forKey: "userId")
                        myGlobalUserId = Int(authorizationResponse.customers![0].id!)
                        UserDefaults.standard.synchronize()
                    ...
                    if let error = error
                    {
                        ...
                        showToast(toastView: self.view, toastTitle: "error", toastText: "incUP".localized())
                        print("mainErr-settings===\(error.localizedDescription)===\(UserDefaults.standard.string(forKey: "AppStatusCode")!)")
