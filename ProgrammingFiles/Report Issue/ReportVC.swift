...
class ReportVC: Base {
    ...
    func setUpView(){
        ...
        descriptionTV.topAnchor.constraint(equalTo: addImageButton.bottomAnchor, constant: 10*iPhoneFactorX).isActive = true
    ...
    @objc func reportIssueButtonAction(){
        ...
        if(descriptionTV.text.count == 0){
            showToast(toastView: self.view, toastTitle: "Error", toastText: "Please enter Description")
            descriptionTV.shake()
            return
        }
        showLoader()
